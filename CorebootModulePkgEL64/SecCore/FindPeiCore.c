 ...
 **/
EFI_STATUS EFIAPI FindImageBase(
	...
	*PeiCoreImageBase = 0;
	...
	for (EndOfFile = CurrentAddress + BootFirmwareVolumePtr->HeaderLength;;) {
		...
		//
		if (IS_FFS_FILE2(File)) {
			EndOfSection = (EFI_PHYSICAL_ADDRESS) (UINTN)(
					(UINT8 *) File + sizeof(EFI_FFS_FILE_HEADER2));
		} else {
			EndOfSection = (EFI_PHYSICAL_ADDRESS) (UINTN)(
					(UINT8 *) File + sizeof(EFI_FFS_FILE_HEADER));
		}
		for (;;) {
 ...
 **/
VOID EFIAPI FindAndReportEntryPoints(
						IN EFI_FIRMWARE_VOLUME_HEADER *BootFirmwareVolumePtr,
