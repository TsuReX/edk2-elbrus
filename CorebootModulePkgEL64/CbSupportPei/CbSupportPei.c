...
**/
VOID BuildMemoryMappedIoRangeHob ( EFI_PHYSICAL_ADDRESS MmioBase,
								   UINT64 				MmioSize ) {
	...
	BuildMemoryAllocationHob ( MmioBase, MmioSize, EfiMemoryMappedIO );
}
...
**/
STATIC BOOLEAN IsFvHeaderValid ( IN EFI_FIRMWARE_VOLUME_HEADER *FwVolHeader ) {
	UINT16 Checksum;
	...
	for (;TempPtr < EndPtr;) {
		if (IsFvHeaderValid ((EFI_FIRMWARE_VOLUME_HEADER* )TempPtr)) {
			if (TempPtr != (UINT8* )(UINTN) PcdGet32 (PcdPayloadFdMemBase))  {
				...
				PeiServicesInstallFvInfoPpi ( NULL,
											  (VOID *) (UINTN) TempPtr,
...
**/
EFI_STATUS CbMemInfoCallback ( UINT64 Base, UINT64 Size, UINT32 Type, VOID *Param ) {
	CB_MEM_INFO	*MemInfo;
	UINTN		Attribue;
...
/**
  ...
  @retval EFI_SUCCESS if it completed successfully.
**/
EFI_STATUS EFIAPI CbPeiEntryPoint ( IN EFI_PEI_FILE_HANDLE		FileHandle,
